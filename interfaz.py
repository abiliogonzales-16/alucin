...
set_appearance_mode("dark")  # Puedes cambiar a "light" si prefieres
set_default_color_theme("green")  # Opciones: "blue", "green", "dark-blue"
...
# Pantalla inicial
# ========================
pantalla_inicial = CTkFrame(app)
pantalla_inicial.pack(fill="both", expand=True)

titulo = CTkLabel(pantalla_inicial, text="Bienvenido ", font=("Arial", 20))
titulo.pack(pady=30)

btn_jugar = CTkButton(pantalla_inicial, text="Jugar", width=200, height=40)
btn_jugar.pack(pady=10)

btn_salir = CTkButton(pantalla_inicial, text="Salir", width=200, height=40, command=app.quit)
...
acciones.pack(pady=10, fill="x", padx=10)
...
# Botón salir en pantalla de juego
CTkButton(pantalla_juego, text="Salir", width=200, command=app.quit).pack(pady=20)
...
def mostrar_juego():
    ...
    pantalla_juego.pack(fill="both", expand=True)
